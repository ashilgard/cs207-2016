...
def lc_reader(filename):
    ...
    with open(filename) as fp:
        facetnames = fp.readline().strip().split()
        facetnames[0] = str(facetnames[0]).replace('#','')
#         print( facetnames)
        facetvalues = fp.readline().strip().split()
        facetvalues[0] = str(facetvalues[0]).replace('#','')
#         print (facetvalues)
        for line in fp:
            if line.find('#')!=0:
                lclist.append([float(f) for f in line.strip().split()])
    dict_of_facets = dict(zip(facetnames,facetvalues))
#     print (dict_of_facets)
